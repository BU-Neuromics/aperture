...
| `cross-class-query.md` | Cross-class query & heterogeneous results (working) | 🟠 Working | Research + candidate design for criteria queries spanning classes: a typed QuerySpec artifact (anchor + quantified relationship conditions), global-search fan-out, joined-table/CSV export semantics, and a `graph` view primitive. Feeds a Proposed ADR slate (0035–0037) and Mosaic asks (X1 promotion, new X3 relationship predicates). |
