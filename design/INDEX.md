...
| `cross-class-query.md` | Cross-class query & heterogeneous results (working) | 🟠 Working | Research + candidate design for criteria queries spanning classes: a typed QuerySpec artifact (anchor + quantified relationship conditions), global-search fan-out, joined-table/CSV export semantics, and a `graph` view primitive. Rev 2 co-designs the Mosaic query surface (priced from mosaic v0.12.1 source): typed filter inputs over CEL-on-the-wire, aggregation/order_by, composed search, `searchAll`/`neighbors` roots, phased relationship predicates. Feeds Aperture ADRs 0035–0037 + Mosaic ADRs 0006–0007 (X1 promotion, new X3). |
