...
| [0035](./decisions/ADR-0035-cross-class-queries-typed-queryspec.md) | Cross-class queries are a typed **QuerySpec** artifact — anchor grain, capability-gated operators, quantified relationship criteria, server-first planner with declared semijoin compensation. **Amended 2026-09-22:** `columns` is built by ADR-0041 (traversal/grain here, visibility view-side); the `rev:`-key note's "mosaic#204 has not merged" is corrected | ✅ Accepted (amended — columns) | `cross-class-query.md` §5–§10; Mosaic ADR-0006/0007; portal req X1/X5; issue #46 |
...
| [0041](./decisions/ADR-0041-referenced-class-slots-projection-vs-presentation.md) | Referenced-class values in the result table are **two** decisions: traversal + grain live in `QuerySpec.columns` (query semantics, server-compilable); visibility/order is a view-side `ColumnView`. Forward to-one/to-many ship client-side now; reverse gates on a declared `inverse:` slot | 🟡 Proposed | 2026-09-22; builds the `columns` field ADR-0035 reserved; Mosaic ADR-0011 verified merged (mosaic#204) — reverse traversal is a schema-authoring act, not a blocker; certification fixture v1.0.0 cannot test explode; issue #65, umbrella datahelix#93 |
