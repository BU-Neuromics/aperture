...
|---|---|---|---|---|
| X1 | Hippo | **Aggregation primitive** — facet/group-by counts, `totalCount`, range filters, `order_by` on the GraphQL list surface. Enables real faceted browse (R3.3r–R3.6). **✅ Promoted** — designed as **Mosaic ADR-0007** ([mosaic#154](https://github.com/BU-Neuromics/mosaic/issues/154), Accepted; implementation staged as OpenSpec `aggregation-and-ordering`, [mosaic#156](https://github.com/BU-Neuromics/mosaic/issues/156)), with the availability-consistency rule pinned (aggregates see exactly what list queries see). Was tracked as hippo#96. | L7 | **High** — top ask; unblocks v1.x faceting |
...
| X5 | Hippo | **Typed operator + relationship-predicate filter contract** — generated per-class `<Type>Filter` input objects (per-slot operator objects `eq/neq/in/gt/gte/lt/lte/contains/isNull` by slot kind/range, `and/or/not`, `where:` arg) with phased relationship predicates (to-one nesting, then to-many `some`/`none` quantifiers); explicitly **not** CEL on the wire, so the introspected schema is the capability contract. Designed as **Mosaic ADR-0006** ([mosaic#153](https://github.com/BU-Neuromics/mosaic/issues/153), Accepted; implementation staged as OpenSpec `typed-filter-inputs`, [mosaic#155](https://github.com/BU-Neuromics/mosaic/issues/155); search composition and heterogeneous roots ride alongside as [mosaic#157](https://github.com/BU-Neuromics/mosaic/issues/157)/[mosaic#158](https://github.com/BU-Neuromics/mosaic/issues/158)). Driven by [ADR-0035](./decisions/ADR-0035-cross-class-queries-typed-queryspec.md) (`cross-class-query.md` §7 names this "X3"; filed as X5 since X3a/X3b were taken). | ADR-0035 | **High** — keystone for cross-class criteria queries |
